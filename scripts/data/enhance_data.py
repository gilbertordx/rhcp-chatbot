...
class DataEnhancer:
    ...
    def enhance_minority_classes(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        threshold_percentage = enhancement_config.get(
        ...
        self.logger.info(
            f"Found {len(minority_classes)} minority classes: {minority_classes}"
        )
        ...
        for intent in minority_classes:
            if intent in templates:
                ...
                self.logger.info(
                    f"Enhancing '{intent}' with {len(examples)} additional examples"
                )
                ...
                self.logger.warning(
                    f"No enhancement templates found for intent: {intent}"
                )
    ...
    def analyze_enhancement_impact(
        self, original_df: pd.DataFrame, enhanced_df: pd.DataFrame
    ) -> dict[str, any]:
        """
        ...
        impact_analysis = {
            ...
            "improvement": {
                ...
                "imbalance_ratio_improvement": (
                    original_analysis["imbalance_ratio"]
                    - enhanced_analysis["imbalance_ratio"]
                ),
...
def main():
    ...
    try:
        ...
        print(
            f"  Percentage increase: {impact['improvement']['percentage_increase']:.1f}%"
        )
        print(
            f"  Imbalance ratio: {impact['original_stats']['imbalance_ratio']:.1f}:1 -> {impact['enhanced_stats']['imbalance_ratio']:.1f}:1"
        )
