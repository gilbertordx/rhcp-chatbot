...
class DataLoader:
    ...
    def load_training_data(self) -> pd.DataFrame:
        ...
        self.logger.info(f"Loading data from {len(training_files)} files: {training_files}")
        ...
        self.logger.info(f"Loaded {len(df)} samples with {df['intent'].nunique()} unique intents")
        
        # Analyze class balance
        balance_analysis = DataUtils.analyze_class_balance(df)
        self.logger.info(f"Class balance analysis:")
        self.logger.info(f"  Most common class: {balance_analysis['most_common_class']} ({balance_analysis['max_samples']} samples)")
        self.logger.info(f"  Least common class: {balance_analysis['least_common_class']} ({balance_analysis['min_samples']} samples)")
        self.logger.info(f"  Imbalance ratio: {balance_analysis['imbalance_ratio']:.2f}:1")
