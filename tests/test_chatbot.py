...
def test_handle_simple_greeting(chatbot_processor):
    ...
    assert any(
        ...
        for word in ["hello", "hey", "hi", "greetings"]
    )
...
def test_album_info_intent(chatbot_processor):
    ...
    response = chatbot_processor.process_message(
        "what albums do they have in their discography"
    )
...
def test_song_info_intent(chatbot_processor):
    ...
    response = chatbot_processor.process_message(
        "what are their most popular songs ever"
    )
...
def test_context_tracking(memory_manager):
    ...
    memory_manager.add_message(
        session_id, "Tell me about Anthony Kiedis", response_with_member
    )
...
def test_conversation_memory_integration(chatbot_processor):
    ...
    # First message
    response1 = chatbot_processor.process_message(
        "Tell me about Anthony Kiedis", session_id
    )
...
def test_session_timeout(memory_manager):
    ...
    short_timeout_manager = ConversationMemory(
        max_sessions=5, session_timeout_hours=0.001
    )  # Very short timeout
...
def test_multiple_sessions(memory_manager):
    ...
    memory_manager.add_message(
        session1, "Hello", {"message": "Hi!", "intent": "greetings.hello"}
    )
    memory_manager.add_message(
        session2,
        "Who is Flea?",
        {"message": "Flea is the bassist", "intent": "member.biography"},
    )
...
def test_chatbot_processor_with_memory(chatbot_processor):
    ...
    response1 = processor_with_memory.process_message(
        "Tell me about Anthony Kiedis", session_id
    )
    response2 = processor_with_memory.process_message(
        "What about his role?", session_id
    )
